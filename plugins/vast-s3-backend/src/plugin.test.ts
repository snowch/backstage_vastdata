...
import { vastS3Plugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [vastS3Plugin],
    });

    await request(server).get('/api/vast-s3/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/vast-s3/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/vast-s3/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/vast-s3/todos/${createdTodoItem.id}`)
  ...
  it('should forward errors from the TodoListService', async () => {
    ...
    const listRes = await request(server).get('/api/vast-s3/todos');
    expect(listRes.status).toBe(401);
    expect(listRes.body).toMatchObject({
      error: { name: 'AuthenticationError' },
    });

    const getRes = await request(server).get('/api/vast-s3/todos/123');
